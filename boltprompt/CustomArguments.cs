...
static class CustomArguments
{
    ...
    private static async Task<string> GetAsync(string command)
    {
        ...
        var result = await Cli.Wrap(commandSplit[0]).WithArguments(commandSplit.Skip(1)).WithValidation(CommandResultValidation.None).ExecuteBufferedAsync();
    ...
    static Suggestion[] ParseOutput(string output, CommandInfo.Argument argument)
    {
        ...
        if (!string.IsNullOrEmpty(argument.CustomCommandRegex))
        {
            ...
            return matches.Select(m => new Suggestion(m.Groups[1].Value)
                { Description = m.Groups.Count > 2 ? m.Groups[2].Value : argDescription }).ToArray();
        }
